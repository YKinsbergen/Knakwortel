...
const entity_1 = require("../admins/entity");
...
let LoginController = class LoginController {
    async authenticate({ email, password }) {
        const admin = await entity_1.default.findOne({ where: { email } });
        if (!admin || !admin.id)
            throw new routing_controllers_1.BadRequestError('A admin with this email does not exist');
        if (!await admin.checkPassword(password))
            throw new routing_controllers_1.BadRequestError('The password is not correct');
        const jwt = jwt_1.sign({ id: admin.id });
